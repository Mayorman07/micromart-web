 ...
 * Connects to the Spring Boot /login endpoint to retrieve JWT and Refresh Token.
 */
const Login = () => {
            ...
            <div className="relative z-10 w-full max-w-md p-10 mx-4 text-center bg-white/95 backdrop-blur-xl rounded-3xl shadow-[0_20px_60px_-15px_rgba(0,180,216,0.3)] border border-white/60">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-5 text-left">
                    <div>
                        ...
                        <input type="email" name="email" value={formData.email} onChange={handleChange} required placeholder="name@example.com" className={inputStyle} />
                    </div>

                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1 tracking-widest">Password</label>
                        <input type="password" name="password" value={formData.password} onChange={handleChange} required placeholder="••••••••" className={inputStyle} />
                    </div>
                    ...
                    <button 
                        ...
                        className={`w-full py-4 rounded-xl font-bold text-white text-lg shadow-xl shadow-cyan-500/10 transition-all duration-200 transform hover:-translate-y-0.5 ${status === 'loading' ? 'bg-slate-300 cursor-not-allowed' : 'bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-700'}`}
                    >
                ...
                <div className="mt-8 pt-6 border-t border-slate-200/60 text-xs text-slate-500 font-medium uppercase tracking-widest">
                    Don't have an account? <button onClick={() => navigate("/signup")} className="text-cyan-600 font-black hover:text-cyan-800 transition-colors ml-1">Create Account</button>
                </div>
            </div>
        </div>
    );
